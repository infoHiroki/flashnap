...
### 連番設定
- デフォルト3桁固定
- 調査ごとに桁数（2〜4）・プレフィックスを設定可能
- 固定仕様: 区切り文字`-`、ゼロ埋めあり、開始番号1

### 健全度調査票
- カテゴリ・部位・材質・特記・判定を写真に紐付け
- 撮影後にメタデータを確認・訂正可能（編集モーダル）
- 最近の撮影・エクスポート画面に判定バッジ表示
- ZIPエクスポート時にmetadata.csv生成、判定別サブフォルダ振り分け
